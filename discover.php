...
function render_discover_card(array $place, array $fav_ids, int $uid): string {
    ...
    $heartBtn = $uid > 0
        ? sprintf('<button class="heart-btn %s" data-id="%d" data-name="%s" data-image="%s" onclick="event.stopPropagation(); toggleFav(this)" title="%s">%s</button>',
            $isFav ? 'faved' : '', $pid, $name, $imgAttr,
    ...
    <div class="place-card%s" id="card-%d" onclick="window.location=\'%s\'">
        ...
        $hasImg ? '' : ' card-no-img', $pid, $link,
        $visual, $overlayClass, $name, $distText, $heartBtn, $futureBar
    );
}
...
<style>
    ...
    /* Heart pinned to fixed bottom-right of card — never moves */
...
function toggleFav(btn) {
    ...
    fetch('toggle_favorite.php', {
        ...
        body: JSON.stringify({ cafe_name: btn.getAttribute('data-name'), cafe_image: btn.getAttribute('data-image'), action: action })
    })
    .then(r => r.json())
    .then(d => { if (d.status !== 'success') { btn.innerHTML = isFaved ? '🤎' : '🤍'; btn.classList.toggle('faved', isFaved); } })
    .catch(() => { btn.innerHTML = isFaved ? '🤎' : '🤍'; btn.classList.toggle('faved', isFaved); });
}
</script>
