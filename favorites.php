...
$stmt = $conn->prepare("SELECT cafe_name, cafe_image FROM favorites WHERE account_id = ? ORDER BY created_at DESC");
        ...
        <?php if($favorites->num_rows > 0): ?>
            <?php while($row = $favorites->fetch_assoc()): ?>
                <div class="place-card">
                    <a href="cafe_window.php?cafe=<?= urlencode($row['cafe_name']) ?>&img=<?= urlencode($row['cafe_image']) ?>" style="text-decoration: none; color: inherit; display: block; height: 100%;">
                        <img src="<?= htmlspecialchars($row['cafe_image']) ?>" alt="<?= htmlspecialchars($row['cafe_name']) ?>" style="height: 200px; object-fit: cover; width: 100%;">
                        <div class="place-name">
                            <span><?= htmlspecialchars($row['cafe_name']) ?></span>
                        </div>
                    </a>
                </div>
            <?php endwhile; ?>
